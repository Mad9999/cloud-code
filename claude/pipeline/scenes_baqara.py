"""Bespoke «الآية تُضيء» scenes for al-Baqara's greatest verses. Each scene is an
...
SRC = BASE / "data" / "scenes_baqara.json"
MORPH = BASE / "data" / "quran-morphology.txt"
OUT = BASE / "app" / "generated" / "scenes_baqara.js"
...
def build():
	...
	for sc in data["verses"]:
		n = sc["n"]
		if n not in counts:
			raise SystemExit(f"scenes_baqara: verse {surah}:{n} not in morphology")
		for k in ("scene_key", "title", "design_rationale", "sources"):
			if not sc.get(k):
				raise SystemExit(f"scenes_baqara: {surah}:{n} missing '{k}'")
		if sc.get("grade") not in GRADES:
			raise SystemExit(f"scenes_baqara: {surah}:{n} bad grade {sc.get('grade')!r}")
		hs = sc.get("hotspots") or []
		if not hs:
			raise SystemExit(f"scenes_baqara: {surah}:{n} has no hotspots")
		for h in hs:
			if not h.get("label") or not h.get("note") or "word" not in h:
				raise SystemExit(f"scenes_baqara: {surah}:{n} hotspot missing word/label/note")
			if not (1 <= h["word"] <= counts[n]):
				raise SystemExit(f"scenes_baqara: {surah}:{n} hotspot word {h['word']} "
					f"out of range 1..{counts[n]}")

	OUT.parent.mkdir(parents=True, exist_ok=True)
	with open(OUT, "w", encoding="utf-8") as f:
		f.write("window.SCENES_BAQARA = ")
		json.dump(data, f, ensure_ascii=False, separators=(",", ":"))
		f.write(";\n")
	print(f"scenes_baqara: {len(data['verses'])} scenes for surah {surah} "
