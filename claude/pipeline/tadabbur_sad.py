"""Progressive sourced contemplation of Sad (surah 38, 88 ayat) — built
...
SRC = BASE / "data" / "tadabbur_sad.json"
MORPH = BASE / "data" / "quran-morphology.txt"
OUT = BASE / "app" / "generated" / "tadabbur_sad.js"
...
def build():
	...
	if data.get("total_ayahs") != TOTAL:
		raise SystemExit(f"tadabbur_sad: total_ayahs must be {TOTAL}")
	...
	for pi, p in enumerate(data["passages"]):
		...
		if nums != list(range(lo, hi + 1)):
			raise SystemExit(f"tadabbur_sad: passage #{pi} '{p.get('title')}' verses "
				f"{nums} do not match range {lo}..{hi}")
		for v in vs:
			n = v["n"]
			if not (1 <= n <= TOTAL):
				raise SystemExit(f"tadabbur_sad: verse {n} out of range")
			if n in seen:
				raise SystemExit(f"tadabbur_sad: verse {n} covered twice")
			seen.add(n)
			ref = v.get("reflection") or {}
			if not ref.get("text") or not ref.get("source"):
				raise SystemExit(f"tadabbur_sad: 3:{n} reflection missing text/source")
			if ref.get("grade") not in GRADES:
				raise SystemExit(f"tadabbur_sad: 3:{n} bad grade {ref.get('grade')!r}")
			if not (v.get("heart_state") or {}).get("text") or not (v.get("action") or {}).get("text"):
				raise SystemExit(f"tadabbur_sad: 3:{n} missing heart_state/action")
			sb = v.get("sabab")
			if sb and (not sb.get("text") or not sb.get("source") or sb.get("grade") not in GRADES):
				raise SystemExit(f"tadabbur_sad: 3:{n} sabab missing text/source/grade")
			covered += 1
		blk = p.get("fadl")
		if blk and (not blk.get("source") or blk.get("grade") not in GRADES):
			raise SystemExit(f"tadabbur_sad: passage #{pi} fadl missing source/grade")

	data["coverage"] = {"covered": covered, "total": TOTAL}
	OUT.parent.mkdir(parents=True, exist_ok=True)
	with open(OUT, "w", encoding="utf-8") as f:
		f.write("window.TADABBUR_SAD = ")
		json.dump(data, f, ensure_ascii=False, separators=(",", ":"))
		f.write(";\n")
	print(f"tadabbur_sad: {len(data['passages'])} passages, {covered}/{TOTAL} ayat covered "
